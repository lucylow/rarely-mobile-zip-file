...
export const MemoryCard = memo(function MemoryCard({ memory, onRemove, onPress }: MemoryCardProps) {
  return (
    <Pressable
      ...
      accessibilityLabel={`${memory.label}: ${memory.value}`}
    ...
    >
      <View style={styles.header}>
        ...
        <Text style={styles.confidence}>{Math.round(memory.confidence * 100)}%</Text>
      </View>
      ...
      <View style={styles.footer}>
        ...
        {onRemove ? (
          <Pressable accessibilityRole="button" accessibilityLabel={`Remove memory about ${memory.value}`} onPress={() => onRemove(memory)} hitSlop={8}>
