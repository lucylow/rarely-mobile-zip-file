...
export function MemorySection({ memories, onRemove }: { memories: MemoryItem[]; onRemove?: (memory: MemoryItem) => void }) {
  return (
    <View style={styles.section}>
      ...
      <Text style={styles.copy}>Small, editable signals—not a score of who you are.</Text>
      {memories.length ? memories.map((memory) => <MemoryCard key={memory.id} memory={memory} onRemove={onRemove} />) : <Text style={styles.empty}>Nothing here yet. Keep using the app normally.</Text>}
    </View>
  );
}
