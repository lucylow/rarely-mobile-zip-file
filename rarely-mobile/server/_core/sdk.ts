...
function decodeStateBase64(state: string): string {
  try {
...
class OAuthService {
  ...
  private decodeState(state: string): string {
    return decodeStateBase64(state);
  }
...
export class SDKServer {
  ...
  async createSessionToken(
    ...
    if (!isNonEmptyString(openId)) {
      throw new Error("openId is required for session token");
    }
  ...
  async signSession(
    ...
    if (!isNonEmptyString(payload.openId) || !isNonEmptyString(payload.appId)) {
      throw new Error("Session payload requires openId and appId");
    }
  ...
  async authenticateRequest(req: Request): Promise<AuthenticatedUser> {
    ...
    if (typeof authHeader === "string" && authHeader.startsWith("Bearer ") && !token) {
      throw ForbiddenError("Invalid authorization header");
    }
